...
function getDecryptedString() {
    ...
    let decrypted = CryptoJS.AES.decrypt(
        encryptData.value,
        CryptoJS.enc.Utf8.parse(atob(key)),
        {
            iv: iv,
...
var file = Uint8Array.from(atob(getDecryptedString()), (c) => c.charCodeAt(0));
...
PDFJS.imageResourcesPath =
    window.url+"/plugins/pdfjs-flipbook/web/images/";
PDFJS.workerSrc =
    window.url+"/plugins/pdfjs-flipbook/build/pdf.worker.js";
PDFJS.cMapUrl = window.url+"/plugins/pdfjs-flipbook/web/cmaps/";
        ...
        return this.initializedPromise.then(
            function () {
                if (DEFAULT_PREFERENCES[name] === undefined) {
                    throw new Error(
                        "preferencesSet: '" + name + "' is undefined."
                    );
        ...
        return this.initializedPromise.then(
            function () {
                ...
                if (defaultValue === undefined) {
                    throw new Error(
                        "preferencesGet: '" + name + "' is undefined."
                    );
                } else {
        ...
        this.initializedPromise = this._readFromStorage().then(
            function (databaseStr) {
                ...
                if (typeof index !== "number") {
                    index =
                        database.files.push({ fingerprint: this.fingerprint }) -
                        1;
                }
    ...
    PDFFindController.prototype = {
        ...
        updateMatch: function PDFFindController_updateMatch(found) {
            ...
            if (found) {
                ...
                state = wrapped
                    ? FindStates.FIND_WRAPPED
                    : FindStates.FIND_FOUND;
...
var SecondaryToolbar = {
        ...
        for (var item in elements) {
            ...
            if (element) {
                element.addEventListener(
                    "click",
                    elements[item].handler.bind(this)
                );
            }
        }
    },
    ...
    setMaxHeight: function secondaryToolbarSetMaxHeight(container) {
        ...
        this.buttonContainer.setAttribute(
            "style",
            "max-height: " +
                (this.newContainerHeight - SCROLLBAR_PADDING) +
                "px;"
        );
...
var HandTool = {
    initialize: function handToolInitialize(options) {
        ...
        if (toggleHandTool) {
            toggleHandTool.addEventListener(
                "click",
    ...
    open: function passwordPromptOpen() {
        OverlayManager.open(this.overlayName).then(
            function () {
                ...
                if (
                    this.reason === PDFJS.PasswordResponses.INCORRECT_PASSWORD
                ) {
            ...
            function TextLayerBuilder_setupRenderLayoutTimer() {
                ...
                var lastScroll =
                    this.lastScrollSource === null
                        ? 0
                        : this.lastScrollSource.lastScroll;
            ...
            return firstPagePromise.then(
                function (pdfPage) {
                    ...
                    for (var pageNum = 1; pageNum <= pagesCount; ++pageNum) {
                        var pageSource = new PDFPageSource(
                            pdfDocument,
                            pageNum
                        );
...
var PDFViewerApplication = {
    ...
    initialize: function pdfViewInitialize() {
        ...
        DocumentProperties.initialize({
            ...
            modificationDateField: document.getElementById(
        ...
        PDFJS.getDocument(
            ...
            function getDocumentError(exception) {
                ...
                if (exception instanceof PDFJS.InvalidPDFException) {
                    ...
                    exception instanceof PDFJS.UnexpectedResponseException
                ) {
                    loadingErrorMessage = mozL10n.get(
    ...
    load: function pdfViewLoad(pdfDocument, scale) {
        ...
        Promise.all(promises).then(function () {
            ...
            pdfDocument.getAttachments().then(function (attachments) {
                ...
                document.getElementById("viewAttachments").disabled =
                    !attachments;

                if (
                    !attachments &&
                    !attachmentsView.classList.contains("hidden")
                ) {
                    self.switchSidebarView("thumbs");
                }
                if (
    ...
    switchSidebarView: function pdfViewSwitchSidebarView(view, openSidebar) {
        ...
        switch (view) {
            case "thumbs":
                var wasAnotherViewVisible =
                    thumbsView.classList.contains("hidden");
    ...
    mouseScroll: function pdfViewMouseScroll(mouseScrollDelta) {
        ...
        if (Math.abs(this.mouseScrollDelta) >= PAGE_FLIP_THRESHOLD) {
            ...
            if (
                (currentPage === 1 &&
                    pageFlipDirection === PageFlipDirection.UP) ||
...
var ThumbnailView = function thumbnailView(
    ...
    this.update = function thumbnailViewUpdate(rotation) {
        ...
        this.canvasHeight =
            (this.canvasWidth / this.pageWidth) * this.pageHeight;
        this.scale = this.canvasWidth / this.pageWidth;
...
window.addEventListener("keydown", function keydown(evt) {
    ...
    if (cmd === 1 || cmd === 8 || cmd === 5 || cmd === 12) {
        ...
        var inPresentationMode =
            pdfViewer &&
            (pdfViewer.presentationModeState ===
                PresentationModeState.CHANGING ||
                pdfViewer.presentationModeState ===
                    PresentationModeState.FULLSCREEN);
    ...
    if (cmd === 0) {
        ...
        switch (evt.keyCode) {
            ...
            case 37: // left arrow
                ...
                if (
                    PDFViewerApplication.pdfViewer.isHorizontalScrollbarEnabled
                ) {
                    break;
                }
            /* falls through */
