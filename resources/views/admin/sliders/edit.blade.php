...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sliders.update", [$slider->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.slider.fields.status') }}</label><br>
                <input type=radio name="status" value="0" {{ $slider->status == '0' ? 'checked' : ''}}> Active<br>
                 <input type=radio name="status" value="1" {{ $slider->status == '1' ? 'checked' : ''}}> Deactive<br>            
            ...
            <div class="form-group">
                ...
                <a class="btn btn-default btn-close" href="{{ route("admin.sliders.index") }}">Cancel</a>
...
@if(isset($slider) && $slider->photo)
      ...
      this.options.thumbnail.call(this, file, '{{ $slider->photo->getUrl('thumb') }}')
