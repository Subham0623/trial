...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="framework">{{ trans('cruds.product.fields.framework') }}</label>
                <input class="form-control {{ $errors->has('framework') ? 'is-invalid' : '' }}" type="text" name="framework" id="framework" value="{{ old('framework', $product->framework) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('published_date') ? 'is-invalid' : '' }}" type="text" name="published_date" id="published_date" value="{{ old('published_date', $product->published_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input type=radio name="featured" value="1" {{ $product->featured == '1' ? 'checked' : ''}}> Yes<br>
                 <input type=radio name="featured" value="0" {{ $product->featured == '0' ? 'checked' : ''}}> No<br>           
            ...
            <!-- dd{{$product->productdetail->overview}} -->
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('authors') ? 'is-invalid' : '' }}" name="authors[]" id="authors" multiple>
                    @foreach($authors as $id => $author)
                        <option value="{{ $id }}" {{ (in_array($id, old('authors', [])) || $product->authors->contains($id)) ? 'selected' : '' }}>{{ $author }}</option>    
                    @endforeach
                </select>
        ...
        @if($errors->has('licenses'))
            ...
            <!-- <div class="form-group">
                <label class="required" for="licenses">{{ trans('cruds.product.fields.license') }}</label><br>
                <table>
                    @foreach($product->licenses as $product_license)
                    @foreach($licenses as $license)
                
                
                
                @if($license->id == $product_license->id)
                <tr>
                    
                        <td><input data-id="{{ $product_license->id }}" type="checkbox" class="license-enable"></td>
                        <td>{{ $product_license->name }}</td>
                        <td><input data-id="{{ $product_license->id }}" name="product_licenses[{{ $product_license->id }}]" type="text" class="license-amount form-control" placeholder="Price" value = "{{$product_license->pivot->price}}"></td>
                    </tr>
                ...
                <tr>
                    ...
                    <td>{{ $license->name }}</td>
                        <td><input data-id="{{ $license->id }}" name="licenses[{{ $license->id }}]" type="text" class="license-amount form-control" placeholder="Price" ></td>
                    </tr>
                    @endif
                    @endforeach
            ...
            <!-- <div class="form-group">
                ...
                <div class="input-group mb-2">
                    ...
                    <div class="form-control d-flex h-auto"> 
                        <span class="text-break" style="flex-grow:1;min-width:0">
                            <a href="{{$product->book->getFirstMedia('book') ? $product->book->getFirstMedia('book')->getUrl(): ''}}">{{$product->book->getFirstMedia('book') ?$product->book->getFirstMedia('book')->name : ''}}</a>
                        </span> 
                    </div>
                </div>
                @endif
                <div class="input-group mb-0">
                    ...
                    <div class="form-control custom-file" style="border:0"> 
                        <input type="file" name="book" class="custom-file-input" accept=".pdf"> 
                        <label class="custom-file-label {{ $errors->has('book') ? 'is-invalid' : '' }} text-truncate"
                            for="id_book">---
                        </label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.product.fields.book_helper') }}</span>
            </div> -->
            
            <!-- <div class="form-group">
                <label class="" for="availToRole">Avail Book To</label><br>
                @foreach($roles as $id => $role)
                <div>
                    <input type="checkbox" name="availToRole[]" value="{{$id}}" 
                    {{ (in_array($id, old('availToRole', [])) || $product->roles->contains($id)) ? 'checked' : '' }}> 
                        {{$role}}
                </div>
                @endforeach
            </div> -->
            ...
            <!-- <div class="form-group">
                ...
                <div class="input-group mb-2">
                    ...
                    <div class="form-control d-flex h-auto"> 
                        <span class="text-break" style="flex-grow:1;min-width:0">
                            <a href="{{$product->manual->getFirstMedia('manual') ? $product->manual->getFirstMedia('manual')->getUrl(): ''}}">{{$product->manual->getFirstMedia('manual') ?$product->manual->getFirstMedia('manual')->name : ''}}</a>
                        </span> 
                    </div>
                </div>
                @endif
                <div class="input-group mb-0">
                    ...
                    <div class="form-control custom-file" style="border:0"> 
                        ...
                        <label class="custom-file-label {{ $errors->has('manual') ? 'is-invalid' : '' }} text-truncate"
                            for="id_manual">---
                        </label>
            ...
            <div class="row">
                <div class="col-md-12 form-group">
                    <label for="resource">Other Resourses</label>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('compatible_browsers') ? 'is-invalid' : '' }}" type="text" name="compatible_browsers" id="compatible_browsers" value="{{ old('compatible_browsers', $product->compatible_browsers) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('software_version') ? 'is-invalid' : '' }}" type="text" name="software_version" id="software_version" value="{{ old('software_version', $product->software_version) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="editor1" name="overview">{!! $product->productdetail->overview !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="editor2" name="features"> {!! $product->productdetail->features !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="editor3" name="requirements">{!! $product->productdetail->requirements !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="editor4" name="instructions">{!! $product->productdetail->instructions !!}</textarea>
