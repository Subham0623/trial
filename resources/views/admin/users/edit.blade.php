    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="radio" name="status" value="1" {{$user->status == 1 ? 'checked' : ''}}> Active<br>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('token') ? 'is-invalid' : '' }}" type="text" name="token" id="token" value="{{ old('token', $user->token) }}" required>
                @if($errors->has('token'))
                    <div class="invalid-feedback">
                        {{ $errors->first('token') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('organizations') ? 'is-invalid' : '' }}" name="organizations[]" id="organizations" multiple>
                    @foreach($organizations as $id => $organization)
                        ...
                        <option value="{{ $id }}" {{ (in_array($id, old('organizations', [])) || $user->organizations->contains($id)) ? 'selected' : '' }}>{{ $organization }}</option>    
                    @endforeach
                </select>
